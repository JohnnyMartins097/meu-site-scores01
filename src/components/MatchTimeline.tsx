...
export const MatchTimeline: React.FC = () => {
  const [matchEvents, setMatchEvents] = useState<TimelineEvent[]>([]);
  const [events, setEvents] = useState<any[]>([]); // Estado 'events' solicitado para futura conexao da API
  ...
  return (
    <div className="relative py-6 px-2 w-full max-w-xl mx-auto overflow-hidden">
      ...
      <div className="relative space-y-8">
        {(matchEvents.length === 0 && events.length === 0) ? (
          <div className="text-center text-gray-500 py-10 font-medium z-10 relative">
            Cronologia minuto-a-minuto indisponível para esta partida.
          </div>
        ) : (
          matchEvents.map((evt) => {
